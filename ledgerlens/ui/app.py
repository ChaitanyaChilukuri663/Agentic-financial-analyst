...
def _try_it() -> None:
    ...
    if st.button("Research it", type="primary"):
        with st.spinner("Reading filings, looking up figures, computing… (first lookup ~30s)"):
            try:
...
def _render_report(report: object) -> None:
    ...
    st.markdown("**How the analyst worked it out** — every figure sourced, every calc verified:")
...
def _how_built() -> None:
    st.markdown("#### The verified-computation pipeline")
    st.graphviz_chart(
        r"""
        digraph {
    ...
    st.markdown(
        "The **Phase-2 agent** wraps this engine as a tool in a *plan → act → observe → revise* "
        "loop, choosing `xbrl_value` / `compute` / `passage` across one or more companies."
    )
    st.divider()
    st.markdown("#### Under the hood")
    st.markdown(
        "- **Executor** — pure-Python interpreter for a small program DSL; the LLM never does "
        "arithmetic (*Program-of-Thoughts*). Reproduces 99.5% of 8,281 FinQA gold programs.\n"
